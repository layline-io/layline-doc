...
](.asset-flow-filterrouting_images/6f1f14ca.png)
...
](.asset-flow-filterrouting_images/91f8dc02.png)
...
](.asset-flow-filterrouting_images/af4bd9e2.png)

Let's add a rule by clicking on `+ ADD AT END`:

](.asset-flow-filterrouting_images/0410396f.png)
...
* **`Evaluation mode`**:
  ...
  ](.asset-flow-filterrouting_images/a38faebb.png)
...
](.asset-flow-filterrouting_images/e025c16d.png)
...
](.asset-flow-filterrouting_images/48e8f672.png)
...
route, you can always use the [Javascript Flow Processor Asset](../../../../sors-flow/asset-flow-javascript) to freely handle message filtering, routing, and whatever else you plan to do with
the message.
