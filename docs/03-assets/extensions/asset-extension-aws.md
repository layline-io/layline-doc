...
3. In your Environment asset, reference the secret:
   ```yaml
   database:
     host: prod-db.cluster-xyz.eu-central-1.rds.amazonaws.com
     password: ${aws:prod/database/password}
   ```

**Result:** When the workflow runs, the `${aws:prod/database/password}` macro resolves to the value stored in AWS Secrets Manager. When the secret rotates, workflows automatically pick up the new value on next execution — no deployment needed.
