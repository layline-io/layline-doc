...
- [**Connection Email**](./connections/asset-connection-email) — SMTP server configuration for sending emails. Use when workflows need to send email notifications, alerts, or reports. Supports TLS/SSL, authentication, and multiple SMTP servers.
...
- [**Output Dev Null**](./processors-output/asset-output-devnull) — Discard output for testing or debugging. Use when you want to test a workflow without actually sending data anywhere, or to measure throughput without I/O overhead.
...
- [**AI Model Resource**](./resources/asset-resource-ai-model) — Deploy and manage machine learning models. Use when using AI Classifier or AI Trainer — defines the model file, version, and runtime parameters. Supports TensorFlow, ONNX, and custom formats.
...
- [**Cassandra Service**](./services/asset-service-cassandra) — Query and update Apache Cassandra databases. Use when Cassandra is your primary data store. Supports CQL queries, batch operations, and asynchronous processing.
...
- [**Virtual File System Service**](./services/asset-service-virtual-fs) — Access virtual or in-memory file systems. Use for temporary storage, caching, or when you need file-like operations without physical storage.
...
- [**Sink Virtual File System**](./sinks/asset-sink-virtual-fs) — Write to virtual or in-memory file systems. Use for testing, temporary storage, or when no physical output is needed.
