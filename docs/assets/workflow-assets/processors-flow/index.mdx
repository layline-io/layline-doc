...
```
Input Processor → [Mapping] → Output Processor
```

### Conditional Routing
```
Input Processor → [Filter & Routing] → (multiple branches) → Output Processors
```

### AI-Powered Classification
```
Input Processor → [AI Trainer - training phase] → AI Storage
Input Processor → [AI Classifier - runtime] → [Filter & Routing] → Output Processors
```

### Rate-Limited Custom Processing
```
Input Processor → [Throttle] → [JavaScript/Python] → Output Processor
```
...
## Flow Processor Reference Documents
