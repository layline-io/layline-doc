...
The Stream class is the underlying class of the [stream](../variables/stream.md) object.
The [stream](../variables/stream.md) instance represents the stream which is being processed at the time and is automatically available for access in any script-based Processor, e.g. the Javascript Processor.
...
Same as [getOriginalName](#getoriginalname).
...
Same as [Stream.originalName](#originalname).
...
> **setProcessorOutputName**(`outputProcessorName`, `streamName`): `Stream`
...
> **setProcessorOutputPath**(`outputProcessorName`, `path`): `Stream`
